...
//! BECAUSE order"). A destructive MQGET is competing-*safe* — two getters never
...
/// The lease key for an MQ source on `queue` — namespaced so it never collides with
...
impl Drop for Lease {
    fn drop(&mut self) {
        ...
        match self.store.delete(&self.key) {
            Ok(()) => eprintln!("[vejas-mq] lease released: {}", self.key),
            Err(e) => eprintln!("[vejas-mq] lease release failed for {}: {e}", self.key),
        }
    }
}
...
pub fn acquire_blocking(
    ...
    let rev = loop {
        ...
        match store.create(key, id.as_bytes()) {
            Ok(r) => {
                eprintln!("[vejas-mq] lease acquired: {key} ({id})");
                break r;
            }
...
mod tests {
    ...
    fn source_key_is_namespaced_and_sanitised() {
        assert_eq!(source_key("DEV.QUEUE.1"), "mq_source_DEV_QUEUE_1");
        assert_eq!(source_key("orders"), "mq_source_orders");
    }
    ...
    fn lease_is_mutually_exclusive() {
        ...
        let key = format!("mq_test_{}", std::process::id());
    ...
    fn stale_revision_is_fenced() {
        ...
        let js = nats::jetstream::new(nc);
        let store = open_store(&js).expect("lease store");
        let key = format!("mq_fence_{}", std::process::id());
        let _ = store.delete(&key);
