...
I have spent most of my career inside integration platforms. ESB flow maps back when the ESB was the center of the world, IoT integrations on Cumulocity, and lately the usual mix of iPaaS and homemade glue that every company runs somewhere. Fifteen years of drawing boxes and arrows, honestly.
...
- Flows are VejasScript files in your own git repository — a small, readable language (a design I have carried since my ESB years, reimplemented in Rust). An agent writes them, a human reviews the pull request, the tests run, GitOps deploys. If you leave, you take your code with you. There is nothing to export because nothing was captured.
- No builder UI. Two screens survive: monitoring (live topology, a per-flow feed of the last processed events, shadow-replay on real traffic) for whoever operates, and a business panel where a domain user reviews and corrects the business surface of a flow (mappings, thresholds). Neither screen can draw a flow. (Metrics are scrapeable at `/metrics` in Prometheus format, and traces export over OTLP/HTTP to any collector you point `OTEL_EXPORTER_OTLP_ENDPOINT` at — both hand-rolled rather than pulled from the `opentelemetry` crate tree, so neither costs the single-digit-megabyte footprint.)
...
The repo is at https://github.com/cpoder/vejas: one Rust runtime, a handful of connectors, and a 53-second recorded demo — a real SAP and a real Salesforce bridged both ways by two readable flows, a domain expert correcting a business rule on screen, shadow-replayed on real events before it lands. The recording is generated by a script that is also a green end-to-end test. If you run integrations for a living and think this is wrong, I want the strongest version of your objection. If you think it is obvious, even better, come help with the connector SDK :)
